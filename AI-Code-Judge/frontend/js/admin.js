...
function renderActivity(items) {
  ...
  items.forEach(j => {
    ...
    div.innerHTML = `
      ...
      <span class="ai-meta">${escapeHtml(j.language)} — user #${j.user_id ?? 'anon'}</span>
...
async function loadUsers(page = 1) {
  ...
  try {
    const data = await Api.get(`/api/admin/users?page=${page}&per_page=20&q=${encodeURIComponent(q)}`);
      ...
      tr.innerHTML = `
        ...
        <td>
          <div style="display:flex;gap:6px;">
            ...
            <button class="btn btn-danger btn-sm delete-user-btn" data-id="${user.id}" data-name="${escapeHtml(user.username)}" title="Delete user">🗑</button>
          </div>
        </td>
      `;
...
async function loadJudgments(page = 1) {
  judgementsPage = page;
  try {
    const data = await Api.get(`/api/admin/judgments?page=${page}&per_page=25`);
